...
@app.command("mix", cls=JsonTyperCommand)
def cmd_mix(
    file: str,
    audio: str = typer.Option(..., help="Path to the audio file to mix in"),
    output: str = typer.Option(..., "-o", "--output", help="Output file path"),
    mix_level: float = typer.Option(
        0.3, help="Volume weight for the mixed audio (0.0–1.0, default 0.3)"
    ),
...
@app.command("volume", cls=JsonTyperCommand)
def cmd_volume(
    file: str,
...
@app.command("replace-audio", cls=JsonTyperCommand)
...
@app.command("normalize", cls=JsonTyperCommand)
def cmd_normalize(
    file: str,
    output: str = typer.Option(..., "-o", "--output", help="Output file path"),
    target_lufs: float = typer.Option(
        -16.0, help="Target integrated loudness in LUFS (default: -16.0)"
    ),
