...
@app.command("fade", cls=JsonTyperCommand)
def cmd_fade(
    file: str,
        ...
        return json_out(
            {
                ...
                "message": str(exc),
                "recovery": [
                    "Use non-negative fade values and ensure total fade fits clip duration"
                ],
            },
            EXIT_VALIDATION,
        )
...
@app.command("text", cls=JsonTyperCommand)
def cmd_text(
    file: str,
    output: str = typer.Option(..., "-o", "--output", help="Output file path"),
    entries_json: Optional[str] = typer.Option(
        None, help="JSON array of text entries (or a single entry object)"
    ),
    entries_file: Optional[str] = typer.Option(
        None, help="Path to a JSON file containing text entries"
    ),
    codec: str = typer.Option("libx264", help="Video codec (default: libx264)"),
) -> int:
...
@app.command("animate", cls=JsonTyperCommand)
def cmd_animate(
    file: str,
    output: str = typer.Option(..., "-o", "--output", help="Output file path"),
    layers_json: Optional[str] = typer.Option(
        None, help="JSON array of animation layer objects (or a single layer object)"
    ),
    layers_file: Optional[str] = typer.Option(
        None, help="Path to a JSON file containing animation layers"
    ),
