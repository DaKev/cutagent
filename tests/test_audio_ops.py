...
class TestMixAudio:
    def test_mix_basic(self, test_video: Any, test_audio_file: Any, output_dir: Any) -> None:
    ...
    def test_mix_low_level(self, test_video: Any, test_audio_file: Any, output_dir: Any) -> None:
...
class TestAdjustVolume:
    def test_volume_boost(self, test_video: Any, output_dir: Any) -> None:
        out = os.path.join(output_dir, "louder.mp4")
        result = adjust_volume(test_video, out, gain_db=6.0)
        assert result.success
        assert os.path.exists(out)

    def test_volume_reduce(self, test_video: Any, output_dir: Any) -> None:
        out = os.path.join(output_dir, "quieter.mp4")
        result = adjust_volume(test_video, out, gain_db=-10.0)
        assert result.success
        assert os.path.exists(out)

    def test_volume_invalid_too_high(self, test_video: Any, output_dir: Any) -> None:
        out = os.path.join(output_dir, "bad.mp4")
        with pytest.raises(CutAgentError) as exc_info:
            adjust_volume(test_video, out, gain_db=100.0)
        assert exc_info.value.code == "INVALID_GAIN_VALUE"

    def test_volume_invalid_too_low(self, test_video: Any, output_dir: Any) -> None:
        out = os.path.join(output_dir, "bad.mp4")
        with pytest.raises(CutAgentError) as exc_info:
            adjust_volume(test_video, out, gain_db=-100.0)
        assert exc_info.value.code == "INVALID_GAIN_VALUE"
...
class TestReplaceAudio:
    def test_replace_basic(self, test_video: Any, test_audio_file: Any, output_dir: Any) -> None:
...
class TestNormalizeAudio:
    def test_normalize_default(self, test_video: Any, output_dir: Any) -> None:
        out = os.path.join(output_dir, "normalized.mp4")
        result = normalize_audio(test_video, out)
        assert result.success
        assert os.path.exists(out)

    def test_normalize_custom_lufs(self, test_video: Any, output_dir: Any) -> None:
        out = os.path.join(output_dir, "normalized_custom.mp4")
        result = normalize_audio(test_video, out, target_lufs=-23.0)
        assert result.success
        assert os.path.exists(out)

    def test_normalize_invalid_lufs_too_high(self, test_video: Any, output_dir: Any) -> None:
        out = os.path.join(output_dir, "bad.mp4")
        with pytest.raises(CutAgentError) as exc_info:
            normalize_audio(test_video, out, target_lufs=0.0)
        assert exc_info.value.code == "INVALID_NORMALIZE_TARGET"

    def test_normalize_invalid_lufs_too_low(self, test_video: Any, output_dir: Any) -> None:
        out = os.path.join(output_dir, "bad.mp4")
        with pytest.raises(CutAgentError) as exc_info:
            normalize_audio(test_video, out, target_lufs=-80.0)
        assert exc_info.value.code == "INVALID_NORMALIZE_TARGET"

    def test_normalize_invalid_peak(self, test_video: Any, output_dir: Any) -> None:
        out = os.path.join(output_dir, "bad.mp4")
        with pytest.raises(CutAgentError) as exc_info:
